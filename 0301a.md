...
Next, execute _perl -V_. We ignore the build information for the moment and focus on the tail end of the output, which is shown in [Example 1](0301ae1.htm).
...
The location of _perl_ is easy since we specified it: /opt/perl/ bin/5.8.0. Make sure that PERL5LIB is unset and execute _/opt/perl/ bin/5.8.0/perl -V_. Again, we are only interested in the tail; see [Example 5](0301ae5.htm).
...
The last lines in perl.sh should set the default Perl environment. [Example 6](0301ae6.htm) shows the specifics of our installation.
