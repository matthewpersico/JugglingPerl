...
_Matthew is currently a vice president in the IT department of Lazard Asset Management in New York City. He can be reached at persicom@acedsl.com._
...
Next, execute _perl -V_. We ignore the build information for the moment and focus on the tail end of the output, which is shown in <a name="re1"></a>[Example 1](0301ae1.md).
...
_delpath_ is a function that will add a directory to a PATH-like environment variable; see [Listing 2](#l2). Detailed discussion of the function is beyond the scope of this article.
...
The location of _perl_ is easy since we specified it: /opt/perl/ bin/5.8.0. Make sure that PERL5LIB is unset and execute _/opt/perl/ bin/5.8.0/perl -V_. Again, we are only interested in the tail; see <a name="re5"></a>[Example 5](0301ae5.md).
...
before all calls to Perl, but that gets tedious after a while and is highly error prone. [Listing 3](#l3) shows the function _swap\__perl with commentary. In order to use the function, simply call it before executing any Perl script:
...
The last lines in perl.sh should set the default Perl environment. <a name="re6"></a>[Example 6](0301ae6.md) shows the specifics of our installation.
